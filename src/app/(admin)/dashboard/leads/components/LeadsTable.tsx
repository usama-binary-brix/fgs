...
const LeadsTable = () => {
  ...
  const handlePromoteClick = async (leadId: any) => {
    try {
      await promoteToInvestor(leadId).unwrap();
      toast.success('Lead promoted to investor successfully!');
      refetch();
    } catch (error) {
      toast.error('Failed to promote lead to investor!');
    }
  };
  ...
  const columns: DataTableColumn[] = [
    ...
    },
    {
      ...
      header: 'Lead Type',
      render: (value) => (
        <span className="text-[14px] text-[#616161] overflow-hidden font-normal font-family capitalize">
          {value || '---'}
        </span>
      ),
      width: '120px'
        ...
        <div className="relative inline-block overflow-visible" ref={openDropdown === row.id ? dropdownRef : null}>
          ...
          {openDropdown === row.id && (
            <div className={`absolute right-9 ${dropdownPosition === 'top' ? 'bottom-full mb-1' : 'top-[-4px] mt-1'} z-[999] w-40 bg-white shadow-md border rounded-sm`}>
              ...
              {row.lead_type === 'investor' && (
                <DropdownItem
                  onItemClick={() => {
                    if (row.type === 'lead') {
                      handlePromoteClick(row.id);
                    }
                    closeDropdown();
                  }}
                  className="flex w-full font-normal !px-4 text-[12px] font-family border-b border-[#E9E9E9] text-[#414141]"
                >
                  {isPromoteLoading && selectedId === row.id ? (
                    "Promoting..."
                  ) : row.type === 'lead' ? (
                    "Promote to investor"
                  ) : (
