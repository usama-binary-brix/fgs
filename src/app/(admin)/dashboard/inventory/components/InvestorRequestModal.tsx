            ...
            <TableHeader className="bg-[#F5F5F5] font-medium font-family text-[12px]">
              <TableRow className='border-b-0'>
                {['NAME', 'EMAIL', 'INVESTMENT AMOUNT', 'STATUS/ACTION'].map((heading) => (
                  <TableCell key={heading} className="px-5 py-3 font-medium text-[#616161] font-family text-start">
            ...
            <TableBody className="divide-y divide-gray-100">
              {filteredInvestments?.map((lead: any, index: number) => (
                <TableRow className='border-b-0 border-t-0' key={index}>
                  <TableCell className="px-5 py-4 text-[13px] border-b-0">{lead.user.first_name} {lead.user.last_name}</TableCell>
                  <TableCell className="px-5 py-4 text-[13px] border-b-0">{lead.user.email}</TableCell>
                  <TableCell className="px-5 py-4 text-[13px] border-b-0">$ {lead.investment_amount}</TableCell>
